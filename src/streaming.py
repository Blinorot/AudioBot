...
def audio_stream(init_queue: mp.Queue, queue: mp.Queue, source,
...
def init_stream(all_models, metadata, device, source="hw:0", format="alsa",
                chunk_size=16000, sample_rate=16000):
    ctx = mp.get_context("spawn")
    manager = ctx.Manager()
    chunk_queue = manager.Queue(maxsize=0)
    stream_init_queue = manager.Queue(maxsize=0)
    streaming_process = ctx.Process(target=audio_stream, args=(stream_init_queue, chunk_queue,
                                                               source,
                                                               format, chunk_size,
                                                               sample_rate))
    ...
    while True:
        try:
            ...
            metadata, user_audio_output = full_handler(all_models, metadata, device, user_full)
